...
forval i =1/12{
	replace emp_growth_sum = emp_growth_sum + L`i'.emp_growth 
	replace obs_num = obs_num +1 if emp_growth~=.
	
		replace emp_growth_sum = emp_growth_sum + F`i'.emp_growth 
	replace obs_num = obs_num +1 if emp_growth~=. 
}
...
forval i =1/12{
	replace dev_mean_sum = dev_mean_sum+ L`i'.dev_mean
	replace dev_mean_sum = dev_mean_sum+ F`i'.dev_mean
}
...
save  "`filepath'/standard_devation.dta"
